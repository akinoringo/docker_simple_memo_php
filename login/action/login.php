<?php
  ...
  if ($statement = $databaseHandler->prepare('SELECT id,name,password FROM users WHERE email = :user_email')) {
    ...
    if (password_verify($user_password, $user['password'])) {
      ...
      if ($statement = $databaseHandler->prepare('SELECT id, title, content FROM memos WHERE user_id = :user_id ORDER BY updated_at DESC LIMIT 1')) {
